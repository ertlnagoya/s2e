...
static uint64_t stm32_timer_read(void *opaque, target_phys_addr_t offset,
                           unsigned size)
{
    Stm32Timer *s = (Stm32Timer *)opaque;
...
static void stm32_timer_write(void * opaque, target_phys_addr_t offset,
...
//    if(s->stm32_rcc_prop == NULL)
  //  {
    ...
    memory_region_init_io(&s->iomem, &stm32_timer_ops, s, "stm32-timer", 0x3ff);
